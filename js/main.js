const validateNameUtf8 = (name) => {
    ...
    return name.match(allowedLettersRegex);
}
...
$(document).ready(() => {
    ...
    const validUsernameInReg = (username, clientsArray) => {
        if (Client.isUsernameExists(username, clientsArray)) {
            if (!$('#unExists').length) { // ako poruka o gresci nije prikazana, prikazi je, inace preskoci dodatni ispis
                $('#usernameReg').after('<span id="unExists" class="red">Username is already taken</span>');
            }
            return false;
        } else {
            $('#unExists').remove();
    ...
    const validEmailInReg = (email, clientsArray) => {
        if (Client.isEmailExists(email, clientsArray)) {
            if (!$('#mailExists').length) {
                $('#mail').after('<span id="mailExists" class="red">E-mail is already taken</span>');
            }
            return false;
        } else {
            $('#mailExists').remove();
    ...
    const validPasswordInReg = (pass) => {        
        if (pass.match(/[a-z]/) && pass.match(/[A-Z]/) && pass.match(/\d/) && pass.length >= 6) {
            ...
            if (!$('#passValidation').length) {
                $('#passwordReg').after('<span id="passValidation" class="red">Password must contains lowercase and capital letter,number and minimum 6 characters</span>');
            }
            return false;            
        }
    }
    ...
    $('#registerBtn').click(() => {
        ...
        if (
            ...
            !validUsernameInReg(username, clientsArray) || 
            !validEmailInReg(email, clientsArray) || 
        ...
        Client.addAndSaveClient(client, clientsArray);

        if (password !== cPassword) {
            $('#cPassword').after('<span class="red">Passwords do not match</span>');
            return false;
        }
        return true;
    ...
    $('#name').keyup(() => {
        if (validateNameUtf8($('#name').val())) {
            ...
            if (!$('#nameValidation').length) {
                $('#name').after('<span id="nameValidation" class="red">Enter letters only</span>');
            }
            return false;            
        }
    });
    ...
    $('#mail').keyup(function () {
        return validEmailInReg($(this).val(), clientsArray);
    });
